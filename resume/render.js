/* =============================================================================
   Renders resume data into the template markup.

   Exposes window.renderResume(data) rather than running once and forgetting,
   so the resume switcher in controls.js can re-render with a different
   application's data without a page reload — the design settings (font,
   size, colors, all driven by CSS custom properties) simply carry over.
...
(function () {
  ...
  function renderResume(data) {
    ...
    // Re-renders start from empty containers so switching resumes never
    // appends a second copy underneath the first.
    header.innerHTML = '';
    main.innerHTML = '';
    aside.innerHTML = '';

    var columns = { main: main, aside: aside };
    ...
    /* A section: indigo heading plus its body. `column` picks which of the two
       ...
       summary, experience, education, skills, tools, recognition — is also
       the order text is extracted from the printed PDF, which is exactly the
       order an ATS wants to read. */
    ...
    /* The browser's own print-to-PDF is what keeps the download an exact
       replica: it writes real vector text, so the file both looks identical
       and stays readable by ATS scanners. The page title becomes the
       suggested filename. */
