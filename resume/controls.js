...
(function () {
  ...
  var RESUME_STORE_KEY = 'resume-active-v1';
  ...
  /* RESUME_LIBRARY is generated by scripts/build-manifest.mjs from the master
     resume plus every file in resume/applications/. Each entry's data object
     is embedded directly (not fetched), so switching works the same way
     whether this page is opened from resume/index.html or from the bundled,
     self-contained artifact — there's no second file to load in either case. */
  var library = window.RESUME_LIBRARY || [];

  if (library.length > 1) {
    var switcher = el('div', 'switcher');
    switcher.appendChild(el('h2', 'panel__title', 'Applying to'));

    var select = el('select', 'field__input switcher__select');
    library.forEach(function (entry) {
      var opt = el('option', null, entry.label);
      opt.value = entry.slug;
      select.appendChild(opt);
    });

    var activeSlug = 'master';
    try {
      var saved = localStorage.getItem(RESUME_STORE_KEY);
      if (saved && library.some(function (e) { return e.slug === saved; })) {
        activeSlug = saved;
      }
    } catch (err) {
    ...
    window.renderResume(
      (library.filter(function (e) { return e.slug === activeSlug; })[0] || library[0]).data
    );

    select.addEventListener('change', function () {
      var entry = library.filter(function (e) { return e.slug === select.value; })[0];
      if (!entry) return;
      window.renderResume(entry.data);
      try {
        localStorage.setItem(RESUME_STORE_KEY, entry.slug);
      } catch (err) {
        /* Private browsing and file:// quotas — not worth breaking over. */
      }
      measureFit();
    });
    ...
    switcher.appendChild(
      el('p', 'panel__hint', 'Design settings below apply to whichever resume is selected here.')
    );
